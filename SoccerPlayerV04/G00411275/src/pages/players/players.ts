    ...
    this.storage.get("dataSettings")
      .then((val) => {
        ...
        this.sp.getPlayers(countryID, minAge, maxAge).subscribe(res => {
          ...
          this.total_dataArray = res['data'];
          console.log(">>> dataArray <<<");
          this.list_length = this.total_dataArray.length;
          this.totalPages = 1 + Math.trunc(this.list_length / this.itensPage);
          this.dataArray = this.total_dataArray.slice(this.page, this.itensPage);
          this.sort();
        });
      });
  }
